...
executor:
  ...
  reranker: none # The reranker to use. Example: 'none', 'local', or 'api'
```
...
> [!NOTE]
> The Test-Workflow Action is the debug version of the main workflow (Send-emails-daily), which retrieves up to 100 papers from the configured two-days-ago arXiv query. The main workflow will be automatically triggered every day and retrieve papers submitted two UTC days ago. There is no new arxiv paper at weekends and holidays, in which case you may see "No new papers found" in the log of main workflow.
...
## 📖 How it works
*Zotero-arXiv-Daily* retrieves papers submitted to arXiv two UTC days ago and, by default, sends them in arXiv submission order without Zotero-based similarity scoring. The TLDR of each paper is generated by LLM from the paper title and abstract, without downloading the arXiv source, HTML, or PDF. If you set `executor.reranker` to `local` or `api`, it loads `data/zotero_corpus.json` when that saved Zotero snapshot exists; otherwise it retrieves the papers in your Zotero library once and saves that snapshot for later runs, then ranks candidates by weighted similarity to your Zotero papers.
