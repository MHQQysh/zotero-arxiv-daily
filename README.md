...
## 📖 How it works
*Zotero-arXiv-Daily* first loads `data/zotero_corpus.json` when that saved Zotero snapshot exists; otherwise it retrieves the papers in your Zotero library once and saves that snapshot for later runs. It then retrieves papers submitted to arXiv two UTC days ago, calculates abstract embeddings, and ranks candidates by weighted similarity to your Zotero papers (newer Zotero papers have higher weight). The TLDR of each paper is generated by LLM from the paper title and abstract, without downloading the arXiv source, HTML, or PDF.
